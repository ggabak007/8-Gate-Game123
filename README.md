...
* PlayerInventory.cs - 플레이어의 도구 사용 로직 + Tools.cs, ToolsType.cs로 사용 도구 관리 **(구현)**
* StageEndTrigger.cs - 씬 이동: 스테이지 간 이동 (성공 시 다음 스테이지, 실패 시 1 스테이지로 이동) , 스테이지 성공/실패 판정 로직 **(구현)**
* GameManager.cs - 씬 데이터 유지 **(구현)**
...
* SceneLoader.cs - 현재 구현해야 할 씬 [맵,엔딩씬,메인메뉴,설정]
* UISettings.cs - UI 관련 작업 [메인메뉴,설정]
...
* AnomalyManager.cs -   확률 기반 변칙 생성 (난수 생성 관리) **(구현)**
* AnomalyObject.cs - 변수에 부착하여 이상현상 부여, 이상현상 물체 해결 확인.  **(일부 구현)** - 이상현상 해결/실패시 실행되는 연출 구현 필요
* GameManager.cs **(구현)** - 스테이지 전환시 자연스럽게 이동하도록 구현함 
* StageEndTrigger.cs **(구현)**


현재 플레이어가 물건을 잡고 떨구는 행동(E,Q키 상호작용), 이상현상 발생(75% 확률), 이상현상을 알맞는 물건으로 해결(다른 물건으로 해결하려고 할시 해결안됨), 해결 후 물건이 파괴(이 부분은 변경가능) 까지 구현하고 테스트 완료했습니다. 


맵 형식 대충 이런식으로 만드시면 될거같습니다!
[alt text](<맵 형식 대충.png>)

빨간선을 밟으면 다음스테이지로 넘어갈지 1스테이지로 넘어갈지 정해지고 하늘색으로 되어있는곳으로 텔레포트가 됩니다 노란색으로 표시한 부분은 자연스럽게 보이기위해서 살짝 빈공간으로 둬야할거같습니다 시작지점이랑 끝지점 맵구성은 같아야합니다